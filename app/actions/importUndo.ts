...
export type UndoResult =
  | { ok: true; deleted: number; kept: number; keptReason: string | null }
...
export async function undoImport(batchId: string): Promise<UndoResult> {
  ...
  if (ids.length === 0)
    return { ok: true, deleted: 0, kept: 0, keptReason: null };
  ...
  return { ok: true, deleted, kept: keptCount, keptReason };
}
